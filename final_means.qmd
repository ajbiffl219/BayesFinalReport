...
Checking different priors for final model (changing means)
...
model {
  ...
  for (i in 1:k) beta[i] ~ normal(b0,ss);
...
lin_data <- list(n = n, k = 1 + length(explainers), y = y, X = X, v = v, ss = 100, b0 = -1)

lin_mod <- stan_model(model_code = lin_model)

all_fit_file <- "best_fit_6m.rda"
overwrite <- FALSE
...
lin_data <- list(n = n, k = 1 + length(explainers), y = y, X = X, v = v, ss = 100, b0 = 0)
...
100*(summary_100 - summary_10)/summary_100
...
100*(summary_100 - summary_1000)/summary_100
...
summary_100[,'sd']/summary_100[,'mean']
